 ...
 * @author martin
 */
@WebServlet(name = "NewFolderServlet", urlPatterns = {"/newFolder.do"})
public class NewFolderServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        String folderName = request.getParameter("txtNewFolder");
        Connector con = (Connector) session.getAttribute("connector");
        ClientPackage cp = (ClientPackage) session.getAttribute("cliPackage");
        con.sendCommand(new Command("@mkd", folderName, cp.getCurrentDir().getCanonicalPath()));
        response.sendRedirect("newFolder.jsp");
    }
